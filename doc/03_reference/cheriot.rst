...
1. **Compile-time**: set the ``BaseIsa`` parameter (see :ref:`parameters`) to ``ibex_pkg::BaseIsaRV32IorCHERIoT``.
...
``data_tag_i``
  ...
  Must be driven high if and only if the loaded word has a stored/valid capability tag.
